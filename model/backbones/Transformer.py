...
class FFusion_trans(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))  # [1,128(+1),embed_dim]
        self.cam_num = camera
        self.view_num = view
    ...
    def forward(self, x, camera_id=None, view_id=None):
        B = x.shape[0]
        x = self.patch_embed(x)  # [32,128,384]

        # cls_tokens = self.cls_token.expand(B, -1, -1)  # cls_tokens:[32, 1, 384]. stole cls_tokens impl from Phil Wang, thanks
...
def vit_base_patch16_224(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
    model = FFusion_trans(
...
def vit_small_patch16_224(img_size=(256, 128), stride_size=16, drop_rate=0., attn_drop_rate=0.,drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
...
def deit_small_patch16_224(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, drop_rate=0.0, attn_drop_rate=0.0, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = FFusion_trans(
